...
add_action( 'plugins_loaded', 'sw_so_scroll_widget_for_eventprime_textdomain' );

function sw_so_scroll_widget_for_eventprime_textdomain() {
  load_plugin_textdomain( 'sw_so_scroll-widget-for-eventprime', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}

function sw_so_add_admin_link( $links ) {
  $admin_link = '<a href="' . esc_url( get_admin_url( null, 'options-general.php?page=sep-scroll-plugin-settings' ) ) . '">' . __( 'Settings', 'sw_so_scroll-widget-for-eventprime' ) . '</a>';
  array_push( $links, $admin_link );
  return $links;
}
add_filter( 'plugin_action_links_scroll-widget-for-eventprime/scroll-widget-for-eventprime.php', 'sw_so_add_admin_link' );
...
function sw_so_scroll_plugin_options() {
  ...
  $options = get_option('sw_so_scroll_plugin_options');
  ...
  $posttypes = array(
    ...
    'em_performer_featured' => 'Performers Featured',
    'em_event' => 'Events'
  );
  ...
  ?>
    <div class="wrap">
      ...
      <h2>ShortCode</h2>
      <p><?php _e( '<code>[sw_for_eventprime]</code>', 'sw_so_scroll-widget-for-eventprime' ); ?></p>
      <form method="post" action="options.php">
        <?php settings_fields('sw_so_scroll_plugin_options_group'); ?>
        <?php do_settings_sections('sw_so_scroll_plugin_settings'); ?>
        <h2><?php _e( 'Shortcode Controll:', 'sw_so_scroll-widget-for-eventprime' ); ?></h2>
...
// check for eventprime-pluginv
function checkIsEventPrimeActive(){
  return is_plugin_active('eventprime-event-calendar-management/event-magic.php') || is_plugin_active('eventprime-event-calendar-management/event-prime.php');
}
...
function sw_so_widget_shortcode($atts) {
  ...
  if(checkIsEventPrimeActive()){
    foreach ( $links as $link ) {
      $output .= '<div class="sw_so_widget-link">' . $link . '</div>';
    }
  }else{
    $output .= '<div class="sw_so_widget-event-magic-not-installed"> Das notwendige Plugin "EventPrime" ist nicht installiert.</div>';
  }
  ...
  $output .= "<style>
      ...
      .sw_so_widget-event-magic-not-installed{
        color: wheat;
...
function sw_so_generate_links($postType) {
  ...
  if($postType == 'em_performer_featured'){
    $args = array(
      ...
      'meta_query' => array(
        array(
          'key' => 'em_is_featured', // Hier den Meta-Schlüssel angeben
          'value' => '1', // Hier den gewünschten Wert angeben
          'compare' => '=', // Vergleichsoperator (hier "gleich")
          'type' => 'NUMERIC', // Datentyp der Meta-Werte
        ),
      ),
    );
  }else{
  ...
  while ($query->have_posts()) {
    ...
    if ($postType == 'em_performer' || $postType == 'em_performer_featured' || $postType == 'em_event') {
